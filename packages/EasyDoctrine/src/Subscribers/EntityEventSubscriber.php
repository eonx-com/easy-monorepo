...
final class EntityEventSubscriber implements EntityEventSubscriberInterface
{
    ...
    public function __construct(
        ...
        #[Deprecated('Since 5.0, will be removed in 6.0', 'Use $subscribedEntities instead')]
        ?array $entities = null,
    ...
    private function dispatchDeferredCollectionUpdates(int $transactionNestingLevel, UnitOfWork $unitOfWork): void
    {
        ...
        foreach ($unitOfWork->getScheduledCollectionUpdates() as $collection) {
            if (
                $collection->getOwner() !== null
                && \in_array($collection->getOwner()::class, $this->subscribedEntities, true)
            ) {
        ...
        foreach ($scheduledCollectionUpdates as $collectionObjectId => $collection) {
            ...
            foreach ($collection->getSnapshot() as $entity) {
                ...
                $snapshotIds[] = $entityId
                    ?? static fn (object $entity): string => (string)$unitOfWork->getSingleIdentifierValue($entity);
            }
    ...
    {
        foreach ($this->filterEntities($unitOfWork->getScheduledEntityDeletions()) as $entity) {
            $changeSet = [];
    ...
    {
        foreach ($this->filterEntities($unitOfWork->getScheduledEntityInsertions()) as $entity) {
    ...
    {
        foreach ($this->filterEntities($unitOfWork->getScheduledEntityUpdates()) as $entity) {
